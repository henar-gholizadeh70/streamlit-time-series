...
if model_type == "ARIMA":
    ...
    arima_option = st.selectbox("Select ARIMA configuration:", ["Select an option", "ARIMA(1,1,1)", "ARIMA(3,1,0)"], key="arima_option_main")
    ...
    if arima_option == "ARIMA(1,1,1)":
        ...
        import matplotlib.pyplot as plt
        fig_arima_111, ax = plt.subplots(figsize=(10, 4))
        ax.plot(y_test.index, y_test, label='Actual')
        ax.plot(y_test.index, forecast, label='Forecast', linestyle='--')
        ax.set_title("ARIMA(1,1,1) Forecast vs Actual")
        ax.legend()
        ax.grid(True)
        st.pyplot(fig_arima_111)
        ...
        import matplotlib.pyplot as plt
        fig_arima_310, ax = plt.subplots(figsize=(10, 4))
        ax.plot(y_test.index, y_test, label='Actual')
        ax.plot(y_test.index, forecast, label='Forecast', linestyle='--')
        ax.set_title("ARIMA(3,1,0) Forecast vs Actual")
        ax.legend()
        ax.grid(True)
        st.pyplot(fig_arima_310)
elif arima_option == "ARIMA(3,1,0)":
        y = participant_data['time_to_event'].astype(float).dropna()
        split_idx = int(len(y) * 0.8)
        y_train, y_test = y[:split_idx], y[split_idx:]
