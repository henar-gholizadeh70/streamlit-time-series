...
# 📌 تست ایستایی با ADF و رسم ACF
st.subheader("📊 Stationarity Test (ADF) & Autocorrelation (ACF)")

selected_series_name = st.selectbox(
    "Select a time series to test for stationarity:",
    ["time_to_event", "blood_pressure_systolic", "blood_pressure_diastolic"]
)

if selected_pid:
    ts_data = grouped.get_group(selected_pid).sort_values('date').set_index('date')[selected_series_name]
    ...
    # نمودار ACF
    st.write(f"### Autocorrelation (ACF) - {selected_series_name} for participant {selected_pid}")
