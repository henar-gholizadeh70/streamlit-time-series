...
if uploaded_file:
    ...
    st.subheader("Stationarity Test (ADF) for time_to_event")
    ...
    if model_type == "ARIMAX":
        st.subheader("ADF Test on Blood Pressure Variables")
        for col in ['blood_pressure_systolic', 'blood_pressure_diastolic']:
            result = adfuller(participant_data[col].dropna())
            st.write(f"{col} - ADF Statistic: {result[0]:.4f}, p-value: {result[1]:.4f}")
            if result[1] >= 0.05:
                st.warning(f"{col} is non-stationary! Consider differencing.")
    ...
    plt.legend()
    plt.title(f"Forecast with model {model_type}")
    st.pyplot(fig3)
