...
# 📌 تست‌های ایستایی برای شرکت‌کننده انتخاب‌شده و سری زمانی انتخابی
st.subheader("📊 Stationarity Analysis (ADF + KPSS + ACF + Differencing)")
...
if selected_pid:
    ...
    try:
        kpss_result = kpss(ts_data.dropna(), regression='c', nlags="auto")
        st.write(f"**KPSS Statistic**: {kpss_result[0]:.4f}")
        st.write(f"**p-value**: {kpss_result[1]:.4f}")
        for key, value in kpss_result[3].items():
            st.write(f"Critical Value ({key}): {value:.4f}")
        if kpss_result[1] > 0.05:
            st.success("✅ Series is stationary according to KPSS (fail to reject H₀)")
        else:
            st.error("❌ Series is non-stationary according to KPSS (reject H₀)")
    except Exception as e:
    ...
    plot_acf(ts_data.dropna(), lags=5)
    plt.title(f"Autocorrelation (ACF) - {selected_series_name} for participant {selected_pid}")
    plt.grid(True)
    plt.tight_layout()
    st.pyplot(fig_acf)
