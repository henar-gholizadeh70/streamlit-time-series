    ...
    try:
        ...
        if participant_series:
            ...
            if model_type == "ARIMAX":
                ...
                st.success("✅ Granger test done!")

                # 🔑 بقیه بخش ARIMAX اصلی اینجا دست نخورده باقی می‌ماند
                # فرض بر این است که کل کد ARIMAX توی فایل اصلی موجود است.

            if model_type == "ARIMA":
                st.subheader("📈 ARIMA Forecasting")
                arima_option = st.selectbox(
                    "Choose ARIMA configuration:",
                    ["Select an option", "ARIMA(1,1,1)"]
                )
